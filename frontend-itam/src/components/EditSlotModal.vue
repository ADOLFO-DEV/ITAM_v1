          ...
          <div class="sm:flex sm:items-start">
            <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left w-full">
              <h3 class="text-xl font-bold leading-6 text-slate-800 mb-6 flex items-center">
                ...
                Activo: <span class="font-mono text-[#F96302] ml-2">{{ formData.telefono }}</span>
              </h3>
              ...
              <div v-show="activeTab === 'editar'" class="animate-in fade-in slide-in-from-right-4 duration-300">
                ...
                <form @submit.prevent="submitForm" class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
                  ...
                  <!-- Empleado -->
                  <div class="sm:col-span-1">
                    <label class="block text-sm font-semibold leading-6 text-slate-700">ID Empleado Asignado</label>
                    <div class="mt-2">
                      <input type="text" v-model="formData.employee_id" class="block w-full rounded-xl border-0 py-2.5 text-slate-900 shadow-sm ring-1 ring-inset ring-slate-200 placeholder:text-slate-400 focus:ring-2 focus:ring-inset focus:ring-[#F96302] sm:text-sm sm:leading-6 px-4 transition-all hover:bg-slate-50/50" />
                    </div>
                  </div>
              ...
              <div v-if="activeTab === 'historial'" class="max-h-96 overflow-y-auto pr-2 animate-in fade-in slide-in-from-left-4 duration-300">
...
watch(() => props.slotData, (newVal) => {
  if (newVal) {
    formData.value = {
      ...
      imei: newVal.imei || '',
      sim: newVal.sim || '',
      centro_costos: newVal.empleado?.centro_costos || '',
      gama: newVal.gama || '',
...
const submitForm = async () => {
  ...
  try {
    const response = await api.patch(`/slots/${formData.value.id}`, {
      ...
      sim: formData.value.sim,
      centro_costos: formData.value.centro_costos,
